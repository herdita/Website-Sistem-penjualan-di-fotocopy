<script src="<?=base_url('admin/transaksi_pembelian/_jquery.min.js')?>"></script>
<script src="<?=base_url('admin/transaksi_pembelian/_typeahead.min.js')?>"></script>
<style>
    .focus-off:focus{
        box-shadow: 0px 0px;
    }
...
<div class="container-fluid">
    ...
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active"> Teransaksi pembelian </li>
    </ol><br>
                    ...
                    <tr>
                        <td> 
                            <div class="input-group">
                                ...
                                <input type="text" name="id_pembelian" value="<?php echo get_uuid('105-',8); ?>" class="form-control" style="width:200px;"/ readonly>      
                            </div>
                        </td>
                        <?php
                        ...
                        <td> 
                            <div class="input-group">
                                <div class="input-group-prepend">
                                    <div class="input-group-text" style="background:#d1ecf1">Tanggal</div>
                                </div>
                                <input type="text" name="tgl_pembelian" value="<?php echo date("Y-m-d"); ?>" class="form-control" style="width:200px;"/ readonly>      
                            </div>
                        </td>
                    </tr>
                </table> <br>
    ...
    $(document).ready(function() {
            ...
            $(this).typeahead({
                source: function(query, result) {
                    $.ajax({
                        url: "<?=base_url('admin/transaksi_pembelian/fetch.php')?>",
                        method: "POST",
                        data: {
                            query: query
                        },
    ...
    <script>
        $(document).ready(function(){
            $(document).on('change','#nama_barang',function(){
                ...
                $("#qualty"+id).prop("disabled", false);
                $.ajax({
                    url:'<?=base_url('admin/transaksi_pembelian/service_harga.php?nama=')?>'+$(this).val(),
                    type:'GET',
                }).done(function(data){
                    var data = JSON.parse(data);
                    // console.log(data);
                    if(data==null){
                        // $("#qualty"+id).prop("disabled", true);
                        $("#qualty"+id).val("");
                        $("#harga"+id).val("");
                        $("#sub_total"+id).val(0);
                        $(document).ready(function(){
            ...
            $('#submit').click(function(){  
                ...
                for (let i = 1; i <= jml; i++){
                    if($('#nama_barang').val() != "" && $('#qualty'+jml).val() != ""){
                        if($('#harga'+jml).val() != "" && $('#sub_total'+i).val() > 0){
                            $.ajax({  
                                url: '<?=base_url('admin/transaksi_pembelian/insert.php')?>',  
                                method:"POST",  
                                data:$('#add_').serialize(),  
                            success:function(data) {  
                                    // alert(data);
                                    mess_success("Data Berhasil Dimasukan","transaksi_pembelian.php");
                                    window.location.href = "?page=transaksi_pembelian";  
                                    // $('#add_')[0].reset();  
                                }  
                            });
                        }else{
